...
async function listAssemblyTemplates(req, res) {
    try {
        ...
        const where = {
            product: { accountGroup: { in: [1402, 1405] } }
        };
...
/**
 * DELETE /api/assembly-templates/:id/destroy
 * Eliminación PERMANENTE de una plantilla inactiva
 */
async function destroyAssemblyTemplate(req, res) {
    try {
        const { id } = req.params;
        const template = await prisma.assemblyTemplate.findUnique({
            where: { id },
            select: { id: true, templateCode: true, isActive: true,
                stages: { select: { id: true } }
            }
        });
        if (!template) return res.status(404).json({ error: 'Plantilla no encontrada' });
        if (template.isActive) {
            return res.status(400).json({ error: 'Desactiva la plantilla antes de eliminarla permanentemente' });
        }
        await prisma.$transaction(async (tx) => {
            const stageIds = template.stages.map(s => s.id);
            if (stageIds.length > 0) {
                await tx.assemblyTemplateStageInput.deleteMany({ where: { stageId: { in: stageIds } } });
                await tx.assemblyTemplateStage.deleteMany({ where: { id: { in: stageIds } } });
            }
            await tx.assemblyTemplate.delete({ where: { id } });
        });
        res.json({ ok: true, deleted: template.templateCode });
    } catch (error) {
        console.error('Error destroying assembly template:', error);
        if (error.code === 'P2025') return res.status(404).json({ error: 'Plantilla no encontrada' });
        res.status(500).json({ error: 'Failed to destroy assembly template' });
    }
}
