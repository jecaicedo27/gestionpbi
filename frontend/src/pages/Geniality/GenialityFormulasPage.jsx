...
import { PlusOutlined, EditOutlined, CheckCircleOutlined, CalculatorOutlined, SearchOutlined, RocketOutlined, DeleteOutlined } from '@ant-design/icons';
...
function GenialityFormulasPage() {
    ...
    const fetchFormulas = async () => {
        ...
        try {
            ...
            const response = await api.get('/geniality/formulas', { params });
    ...
    const handleDelete = async (formulaId) => {
        try {
            await api.delete(`/geniality/formulas/${formulaId}`);
            message.success('Formulación eliminada');
            fetchFormulas();
        } catch (error) {
            console.error('Error deleting formula:', error);
            message.error(error.response?.data?.error || 'Error al eliminar formulación');
        }
    };
    ...
    const handleApprove = async (formulaId) => {
        try {
            ...
            await api.post(`/geniality/formulas/${formulaId}/approve`, {
                approvedById: userId
            });
            ...
            render: (_, record) => (
                <Space size="small">
                    <Button
                        type="link"
                        icon={<EditOutlined />}
                        onClick={() => navigate(`/geniality/formulas/${record.id}`)}
                    >
                        Editar
                    </Button>
                    {!record.approvedAt && (
                    ...
                    <Tooltip title="Recalcular Costos">
                        <Button
                            ...
                            onClick={() => {
                                api.post(`/geniality/formulas/${record.id}/calculate-cost`).then(() => {
                    ...
                    <Popconfirm
                        ...
                        description={`Se eliminará "${record.formulaName}" permanentemente.`}
            ...
            <Card
                ...
                extra={
                    <Button
                        type="primary"
                        icon={<PlusOutlined />}
                        onClick={() => navigate('/geniality/formulas/new')}
                    >
                        Nueva Formulación
                    </Button>
                }
...
export default GenialityFormulasPage;
