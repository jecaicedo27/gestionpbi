...
function GenialityTemplateEditorPage() {
    const { id } = useParams();
    ...
    const loadData = async () => {
        ...
        try {
            // Load process types
            const processTypesRes = await api.get('/geniality/process-types', {
                params: { active: true }
            });
            setProcessTypes(processTypesRes.data);

            // Load products
            const productsRes = await api.get('/geniality/products');
            setProducts(productsRes.data);

            // Load formulas
            const formulasRes = await api.get('/geniality/formulas');
            setFormulas(formulasRes.data);

            // Always load all templates for indicator
            const allTemplatesRes = await api.get('/geniality/assembly-templates');
            setAllTemplates(allTemplatesRes.data || []);

            // Load template if editing
            if (id && id !== 'new') {
                const templateRes = await api.get(`/geniality/assembly-templates/${id}`);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (id && id !== 'new') {
                await api.patch(`/geniality/assembly-templates/${id}`, payload);
                message.success('Plantilla actualizada exitosamente');
            } else {
                const response = await api.post('/geniality/assembly-templates', payload);
                message.success('Plantilla creada exitosamente');
                navigate(`/geniality/assembly-templates/${response.data.id}`);
            }
        } catch (error) {
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div>
                                <Button
                                    ...
                                    onClick={() => navigate('/geniality/assembly-templates')}
                                    style={{ marginRight: 8 }}
                                >
                                    Volver
                                </Button>
...
export default GenialityTemplateEditorPage;
