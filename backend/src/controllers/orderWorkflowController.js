...
exports.completePickingWithBackorder = async (req, res) => {
    try {
        ...
        const { updatedOrder, backorder } = await prisma.$transaction(async (tx) => {
            ...
            const facturadoDetail = itemScanned
                .filter(i => i.scannedQty > 0)
                .map(i => `${i.productName} ${i.scannedQty}/${i.requestedQty}`)
                .join(', ');
            const faltanteDetail = deficitItems
                .map(i => `${i.productName} x${i.requestedQty - i.scannedQty}`)
                .join(', ');

            // 4. Generate unique backorder number based on original (e.g. ORD-COMERCIAL-31032026-2-BKO1)
            const baseNumber = order.orderNumber;
            const existingBKOs = await tx.order.count({
                where: { orderNumber: { startsWith: `${baseNumber}-BKO` } }
            });
            const newOrderNumber = `${baseNumber}-BKO${existingBKOs + 1}`;
            ...
            const originalNotes = [
                order.notes || '',
                `[Parcial ${pickingPct}%]`,
                `Facturado: ${facturadoDetail}`,
                `Faltantes movidos a → ${newOrderNumber}`,
            ].filter(Boolean).join(' | ');
